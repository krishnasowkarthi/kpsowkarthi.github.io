...
void loop() {
  ...
  if(counting_flag == 0) {
    ...
    if (flowRate > 0) {                //To check if pipe is open
      if (totalMilliLitres <= 1000) {  //to check if used litres exceeds
  ...
  if (radio.available()){
    ...
    char rx_Char = rx_msg[0];                 //for reading incoming msg from main unit
    if(rx_Char == '%') {
      total_tx_flag = 1;
    }
    if (rx_msg[0] != 2){
  ...
  while(warning_tx_flag == 1) {
    ...
    for (int i = -1; i < tx_messageSize; i++) {
      int charToSend[1];
      charToSend[0] = tx_Message.charAt(i);      //sending msg to main unit
